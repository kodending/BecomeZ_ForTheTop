...
public class Player_Dodge_Move : PlayerBaseState
{
    [Range(0f, 2f)] public float speedMultiplier = 1f;

    public Player_Dodge_Move(InGamePlayerController pc) : base(pc) { }

    public override void OnEnterState()
    {
        playerController.m_agent.enabled = false;

        playerController.m_pv.RPC("AnimTriggerRPC", RpcTarget.All, "Dodge_Move");
    }

    public override void OnUpdateState()
    {
        if (playerController.m_anim.GetCurrentAnimatorStateInfo(0).IsName("Dodge_Move"))
        {
